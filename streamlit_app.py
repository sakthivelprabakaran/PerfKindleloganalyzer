...
with col1:
        ...
        with b_col2:
            if st.button("🔄 Process All", use_container_width=True, disabled=not st.session_state.all_iterations_data):
                with st.spinner("Processing iterations..."):
                    mode_map = {"Default (Button Up)": "default", "Swipe (Button Down)": "swipe", "Suspend (Power Button)": "suspend"}
                    ...
                    for i in range(0, len(iterations), 2):
                        if i + 1 < len(iterations):
                            iteration_pairs.append((iterations[i], iterations[i+1]))
                    results = []
    ...
    if processing_mode == "Single Entry":
        export_format_container = st.container()
        with export_format_container:
            pdf_checked = st.checkbox("PDF", value=True)
            txt_checked = st.checkbox("TXT", value=True)

        if st.button("Export Report", disabled=not results_exist):
            if pdf_checked:
                pdf_exporter = PdfExporter()
                st.session_state.pdf_export_bytes = pdf_exporter.generate_pdf_bytes(st.session_state.results, st.session_state.calculation_mode)
            if txt_checked:
                txt_exporter = TxtExporter()
                st.session_state.txt_export_bytes = txt_exporter.generate_txt_bytes(st.session_state.results)

        if 'pdf_export_bytes' in st.session_state and st.session_state.pdf_export_bytes:
            st.download_button(label="Download PDF Report", data=st.session_state.pdf_export_bytes, file_name=f"{st.session_state.test_case_name or 'report'}.pdf", mime="application/pdf", on_click=lambda: st.session_state.pop('pdf_export_bytes', None))
        if 'txt_export_bytes' in st.session_state and st.session_state.txt_export_bytes:
            st.download_button(label="Download TXT Report", data=st.session_state.txt_export_bytes, file_name=f"{st.session_state.test_case_name or 'report'}.txt", mime="text/plain", on_click=lambda: st.session_state.pop('txt_export_bytes', None))
        ...
        if results_exist:
            ...
            st.download_button(label="Export All Reports (ZIP)", data=zip_buffer.getvalue(), file_name="kindle_batch_reports.zip", mime="application/zip")
        else:
            st.button("Export All Reports (ZIP)", disabled=True)

        if results_exist:
            excel_exporter = ExcelExporter()
            excel_bytes = excel_exporter.generate_excel_bytes(st.session_state.batch_results)
            st.download_button(label="Export Excel Report", data=excel_bytes, file_name="kindle_batch_analysis.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        else:
...
with col2:
    if not st.session_state.results and not st.session_state.batch_results:
        ...
        tabs = st.tabs(["📊 Summary", "📋 Main Results", "📦 Waveform Boxes", "📏 Heights & Waveforms", "📁 Batch Results", "⚖️ Comparison"])
        tab1, tab2, tab3, tab4, tab5, tab6 = tabs

        with tab1:
            st.header("Processing Summary")
            # ... (rest of the tab logic remains the same)
            def generate_summary_html(results, test_case_name):
                if not results: return "<p>No results to summarize.</p>"
                total_iterations = len(results)
                durations = [r['duration'] for r in results]
                avg_duration = sum(durations) / total_iterations
                min_duration = min(durations)
                max_duration = max(durations)
                summary_html = f"""<h4>📊 Summary for {test_case_name}</h4><table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width:100%;"><tr><td><b>Test Case:</b></td><td>{test_case_name}</td></tr><tr><td><b>Processing Mode:</b></td><td>{st.session_state.calculation_mode}</td></tr><tr><td><b>Total Iterations:</b></td><td>{total_iterations}</td></tr><tr><td><b>Average Duration:</b></td><td style="background-color: yellow;">{avg_duration:.3f} s</td></tr><tr><td><b>Min Duration:</b></td><td>{min_duration:.3f} s</td></tr><tr><td><b>Max Duration:</b></td><td>{max_duration:.3f} s</td></tr></table><br>"""
                return summary_html
            if st.session_state.processing_mode == "Single Entry":
                if st.session_state.results:
                    st.markdown(generate_summary_html(st.session_state.results, st.session_state.test_case_name or "Single Entry"), unsafe_allow_html=True)
                else: st.info("No data to display.")
            else:
                if st.session_state.batch_results:
                    for filename, results in st.session_state.batch_results.items():
                        st.markdown(generate_summary_html(results, filename), unsafe_allow_html=True)
                        st.markdown("---")
                else: st.info("No data to display.")
        ...
        with tab2:
            ...
            def results_to_html(results):
                ...
                df_display = df[['iteration', 'duration', 'start', 'stop', 'marker', 'max_height', 'max_height_waveform', 'mode']]
                return df_display.to_html(classes='dataframe', index=False)

            if st.session_state.processing_mode == "Single Entry":
                if st.session_state.results:
                    st.markdown(results_to_html(st.session_state.results), unsafe_allow_html=True)
                else:
                ...
                if st.session_state.batch_results:
                    for filename, results in st.session_state.batch_results.items():
                        ...
                        if results:
                            st.markdown(results_to_html(results), unsafe_allow_html=True)
                        else:
        ...
        with tab4:
            ...
            def heights_to_html(results):
                ...
                if not all_heights_data:
                    return ""
                df = pd.DataFrame(all_heights_data)
                return df.to_html(classes='dataframe', index=False)

            if st.session_state.processing_mode == "Single Entry":
                if st.session_state.results:
                    st.markdown(heights_to_html(st.session_state.results), unsafe_allow_html=True)
                else:
                ...
                if st.session_state.batch_results:
                    for filename, results in st.session_state.batch_results.items():
                        ...
                        if results:
                            st.markdown(heights_to_html(results), unsafe_allow_html=True)
                        else:
                            st.warning("No valid results found for this file.")
                else:
                    st.info("No data to display.")
        ...
        with tab6:
            ...
            if 'comparison_html' not in st.session_state: st.session_state.comparison_html = ""
            def generate_comparison_html(result_a, result_b):
                duration_a, duration_b = result_a['duration'], result_b['duration']
                duration_diff = duration_b - duration_a
                if duration_a > 0:
                    deviation, (verdict, color) = (duration_diff / duration_a) * 100, ("Slower", "red") if duration_diff > 0 else ("Faster", "green")
                    verdict_text = f"{verdict} ({deviation:+.2f}%)"
                else: verdict_text, color = "N/A", "black"
                html = f"""<h4>Comparison Summary</h4><table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width:100%;"><tr style="background-color: #f0f0f0;"><th>Metric</th><th>Log A (Previous)</th><th>Log B (Current)</th><th>Difference</th><th style="color: {color};">Deviation</th></tr><tr><td><b>Duration (s)</b></td><td>{duration_a:.3f}</td><td>{duration_b:.3f}</td><td>{duration_diff:+.3f}</td><td style="color: {color};"><b>{verdict_text}</b></td></tr></table><h4>Waveform Pattern Comparison</h4>"""
                seq_a, seq_b = [(h['height'], h['waveform']) for h in result_a['all_heights']], [(h['height'], h['waveform']) for h in result_b['all_heights']]
                table_html = """<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;"><tr style="background-color: #f0f0f0;"><th>Step</th><th>Log A (Height, Waveform)</th><th>Log B (Height, Waveform)</th><th>Comparison</th></tr>"""
                matcher = difflib.SequenceMatcher(None, seq_a, seq_b)
                step = 1
                for tag, i1, i2, j1, j2 in matcher.get_opcodes():
                    if tag == 'equal':
                        for i in range(i1, i2): table_html += f'<tr><td>{step}</td><td>{seq_a[i][0]}px, {seq_a[i][1]}</td><td>{seq_b[j1 + (i - i1)][0]}px, {seq_b[j1 + (i - i1)][1]}</td><td style="color: green;">Identical</td></tr>'; step += 1
                    if tag == 'delete':
                        for i in range(i1, i2): table_html += f'<tr style="background-color: #ffcccb;"><td>{step}</td><td>{seq_a[i][0]}px, {seq_a[i][1]}</td><td>-</td><td style="color: red;"><b>Removed from Log B</b></td></tr>'; step += 1
                    if tag == 'insert':
                        for j in range(j1, j2): table_html += f'<tr style="background-color: #ccffcc;"><td>{step}</td><td>-</td><td>{seq_b[j][0]}px, {seq_b[j][1]}</td><td style="color: blue;"><b>New in Log B</b></td></tr>'; step += 1
                    if tag == 'replace':
                        for i, j in zip(range(i1, i2), range(j1, j2)): table_html += f'<tr style="background-color: #ffffcc;"><td>{step}</td><td>{seq_a[i][0]}px, {seq_a[i][1]}</td><td>{seq_b[j][0]}px, {seq_b[j][1]}</td><td style="color: orange;"><b>Deviation</b></td></tr>'; step += 1
                table_html += "</table>"
                html += table_html
                return html
            if st.button("⚖️ Compare Logs", use_container_width=True):
                if log_a and log_b:
                    with st.spinner("Comparing logs..."):
                        mode_map = {"Default (Button Up)": "default", "Swipe (Button Down)": "swipe", "Suspend (Power Button)": "suspend"}
                        current_mode = mode_map.get(st.session_state.calculation_mode, "default")
                        result_a, result_b = process_log_iteration_data(log_a.split('\n'), "A", current_mode), process_log_iteration_data(log_b.split('\n'), "B", current_mode)
                        if result_a and result_b: st.session_state.comparison_html = generate_comparison_html(result_a, result_b)
                        else: st.session_state.comparison_html = "<p style='color:red;'>Could not process one or both logs. Please ensure they are valid single iterations.</p>"
                else: st.warning("Please paste log content into both Log A and Log B inputs.")
