...
class StateManager:
    ...
    def add_recent_session(self, session_data):
        """Adds a session to the list if it's not already there."""
        # Avoid duplicates
        for session in self.sessions:
            if session['file_name'] == session_data['file_name'] and session['project_path'] == session_data['project_path']:
                return
